...
// ガーベージコレクションが削除するセッションの有効期限を設定（30日以上経っているものに対してだけ、100分の1の確率で、削除する）
...
function validRequired($str, $key){
    if($str === ''){ //金額フォームを考えると数値の０はOKにし、空文字はダメにする
        global $err_msg;
        $err_msg[$key] = MSG01;
    }
}
...
function dbConnect(){
    ...
    $options = array(
        ...
        PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,  //ここが;になってた
...
function getProductList($currentMinNum = 1,$category,$sort,$span = 5){
    debug('商品情報を取得します。');
    //例外処理
    try{
        // DBへ接続
...
function getMsgsAndBord($id){
    ...
    try{
        ...
        $dbh = dbConnect();
        //SQL文接続
        $sql = 'SELECT m.id AS m_id,m.delete_flg ,product_id, bord_id, send_date, to_user, from_user, sale_user, buy_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b ON b.id = m.bord_id WHERE b.id = :id ORDER BY send_date ASC';
        $data = array(':id' => $id);
        // クエリ実行
        $stmt = queryPost($dbh, $sql, $data);
  ...
  try {
    ...
    if(!empty($rst)){
      foreach($rst as $key => $val){
        ...
        //ここの多次元配列が理解
        //新しく['msg']というvalueの中のkeyに作り、そこにまた、連想配列の形で、データを入れている。
       }
     }
//   
//      if(!empty($rst[$key]['msg'])){
//        foreach($rst[$i]['msg'] as $key => $val){
//          // SQL文作成
//          $sql = 'SELECT * FROM users WHERE id = :id AND delete_flg = 0';
//          $data = array(':id' => $val['to_user']);
//          クエリ実行
//          $stmt = queryPost($dbh, $sql, $data);
//          $rst[$key][$key]['userinfo'] = $stmt->fetchAll();
//        }
//      }
...
function getFormData($str, $flg = false){
    ...
    global $dbFormData;  //getUserで取得したレコード情報代入した変数と比べる。
    // ユーザーデータがある場合
    if(!empty($dbFormData)){
        //フォームのエラーがある場合
        if(!empty($err_msg[$str])){
            ...
            }else{ //ない場合、そもそも未入力はバリデーションに引っかかるため。
                return sanitize($dbFormData[$str]);
            }
        }else{
            //POST通信にあるが、DBの情報と異なる場合。（他OK。でも他ダメ）
